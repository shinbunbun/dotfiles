...
> **Note**: 監視・ログ分析スタック（Prometheus、Loki、Fluent Bit等）は[nixos-observability](https://github.com/shinbunbun/nixos-observability)/[nixos-observability-config](https://github.com/shinbunbun/nixos-observability-config)に移行しています。可視化（Grafana）は [k8s-apps](https://github.com/shinbunbun/k8s-apps) で k3s 上にホストされています。
...
収集側（Prometheus / Loki / Alertmanager / Fluent Bit / Node Exporter / SNMP Exporter）は [nixos-observability](https://github.com/shinbunbun/nixos-observability) / [nixos-observability-config](https://github.com/shinbunbun/nixos-observability-config) で管理し、本リポジトリから flake input として参照しています。可視化（Grafana）は [k8s-apps](https://github.com/shinbunbun/k8s-apps) 配下で k3s 上にホストされ、NixOS 側の Prometheus / Loki をデータソースとして参照します。

### アーキテクチャ

```
各ホスト → Fluent Bit → Loki(homeMachine) ─┐
                                            ├─→ Grafana(k3s)
              Prometheus(homeMachine) ──────┘
                        ↑
              Node Exporter（各ホスト）
```
...
物理ホスト・RouterOS・k3s ノード間のネットワーク構成図は [Scanopy](https://scanopy.net) (Community Edition、AGPL-3.0) を k3s 上に self-host して**自動生成**しています。LAN (192.168.1.0/24) と Cilium LB VIP レンジ (192.168.128.0/24) を継続ディスカバリしてトポロジを更新します。

- **UI (外部)**: https://scanopy.shinbunbun.com (Authentik SSO)
- **UI (LAN)**: http://192.168.128.16:60072
- **デプロイ**: [k8s-apps/apps/scanopy](https://github.com/shinbunbun/k8s-apps/tree/main/apps/scanopy) (ArgoCD 管理)
- **スキャン対象**: LAN 各ホスト、RouterOS (SNMP v2c)、k3s ノード

k8s アプリ層・GitOps フロー・データフローの図は対象外です (必要なら別途 Mermaid で補完)。
